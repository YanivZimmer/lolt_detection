...
def benchmark_rf_inference(model: RandomForestModel, events: List[Dict[str, Any]], 
                          feature_extractor: ComprehensiveFeatureExtractor,
                          feature_names: List[str], n_iterations: int = 10) -> Dict[str, float]:
    """
...
def main():
    ...
    parser.add_argument('--output', type=str, default='inference_benchmark.json',
    ...
    print(f"  Average: {(metrics['avg_per_event_ms']+metrics['avg_feature_extraction_ms']):.4f} ms per event total process time")
    ...
    speedup = claude_latency_ms / (metrics['avg_per_event_ms']+metrics['avg_feature_extraction_ms'])
    
    print(f"\n" + "="*60)
    print("Comparison with Claude-Sonnet-4.5 Baseline")
    print("="*60)
    print(f"Claude latency (estimated): {claude_latency_ms} ms per event")
    print(f"Our fe+latency: {(metrics['avg_per_event_ms']+metrics['avg_feature_extraction_ms']):.4f} ms per event")
