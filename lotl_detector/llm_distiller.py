...
class LLMDistiller:
    ...
    def extract_reasoning_features(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        features["reasoning_mentions_obfuscation"] = (
            1
            if any(word in reason_lower for word in ["obfuscat", "encod", "hidden", "stealth"])
            else 0
        )

        features["reasoning_mentions_suspicious"] = (
            1
            if any(word in reason_lower for word in ["suspicious", "unusual", "anomal", "abnormal"])
            else 0
        )

        features["reasoning_mentions_legitimate"] = (
            1
            if any(word in reason_lower for word in ["legitimate", "normal", "typical", "expected"])
            else 0
        )

        features["reasoning_mentions_system"] = (
            1
            if any(word in reason_lower for word in ["system", "binary", "native", "built-in"])
            else 0
        )

        features["reasoning_mentions_network"] = (
            1
            if any(word in reason_lower for word in ["network", "download", "connect", "remote"])
            else 0
        )

        features["reasoning_mentions_execution"] = (
            1 if any(word in reason_lower for word in ["execut", "spawn", "launch", "run"]) else 0
        )
    ...
    def generate_explanation(self, event: Dict[str, Any], rf_fe: str, predicted_label: str) -> str:
        ...
        if predicted_label == "malicious":
            ...
            if "Medium" in integrity or "High" in integrity:
                reasons.append(
                    f"Executed with appropriate integrity level. integrity is {integrity}"
                )
