...
class LOTLEnsemble:
    ...
    def predict_with_explanation(self, events: List[Dict[str, Any]], 
                                 use_llm_explanation: bool = False) -> List[Dict[str, Any]]:
        """
        ...
        for i, (event, pred) in enumerate(zip(events, predictions)):
            ...
            rf_explanation = self._generate_rf_explanation(
            ...
            if use_llm_explanation and self.use_llm_reasoning and self.llm_distiller:
                try:
                    llm_explanation = self.llm_distiller.generate_explanation(event,rf_explanation, pred)
                except Exception as e:
                    print(f"Warning: Could not generate LLM explanation: {e} {traceback.format_exc()}")
    ...
    def _generate_rf_explanation(self, event: Dict[str, Any], prediction: str, 
        ...
        if prediction == 'malicious':
            ...
            for i, feat in enumerate(top_5_features, 1):
                feat_name = feat['name']
                feat_value = feat['value']
                feat_explanation = self._explain_feature(feat_name, feat_value, feat['contribution'], event, is_benign=True)
                if feat_explanation:
                    explanation_parts.append(f"{i}. {feat_explanation}")
        
        return "\n".join(explanation_parts)
    
    def _explain_feature(self, feat_name: str, feat_value: float, contribution: float,
                        event: Dict[str, Any], is_benign: bool = False) -> str:
        """
        ...
        if 'explorer_from_userinit' in feat_name and feat_value > 0:
            explanations.append(f"**Explorer from userinit.exe** (value: {value_str}, SHAP: {contribution_str}): Windows Explorer launched by userinit.exe - suspicious as Explorer should be executed by user GUI, indicating potential lateral movement.")
            ...
            explanations.append(f"**System binary from Explorer** (value: {value_str}, SHAP: {contribution_str}): {exe_name} launched from Explorer - unusual as system binaries typically run from system processes.")
        
        elif 'suspicious_parent_child' in feat_name and feat_value > 0:
            explanations.append(f"**Suspicious parent-child process** (value: {value_str}, SHAP: {contribution_str}): Unusual process relationship indicating potential lateral movement or process injection.")
        
        # Obfuscation features
        elif feat_name.startswith('obfuscation_') and feat_value > 0:
            obf_type = feat_name.replace('obfuscation_', '').replace('_', ' ')
            explanations.append(f"**{obf_type.title()} obfuscation detected** (value: {value_str}, SHAP: {contribution_str}): Command contains {obf_type} patterns used to hide malicious intent.")
        
        elif 'cmdline_entropy' in feat_name:
            if feat_value > 4.5:
                explanations.append(f"**High command-line entropy** (value: {value_str}, SHAP: {contribution_str}): High randomness suggests obfuscation or encoding in the command.")
            else:
                explanations.append(f"**Command-line entropy** (value: {value_str}, SHAP: {contribution_str}): Normal entropy level.")
        
        # System account features
        elif 'is_system_user' in feat_name and feat_value > 0:
            user = event.get('User', 'N/A')
            explanations.append(f"**System account execution** (value: {value_str}, SHAP: {contribution_str}): Process running as {user} - unusual for interactive commands, may indicate privilege escalation.")
        
        elif 'system_account_admin_tool' in feat_name and feat_value > 0:
            explanations.append(f"**System account using admin tools** (value: {value_str}, SHAP: {contribution_str}): System service account using administrative tools - anomalous pattern.")
        
        # Path features
        elif 'suspicious_path_operation' in feat_name and feat_value > 0:
            explanations.append(f"**Suspicious path operation** (value: {value_str}, SHAP: {contribution_str}): Command operates on files in suspicious locations (Public Downloads, Temp) commonly used for malware staging.")
        
        elif 'system_file_modification' in feat_name and feat_value > 0:
            explanations.append(f"**System file modification attempt** (value: {value_str}, SHAP: {contribution_str}): Attempts to modify critical system files (e.g., hosts file) - defense evasion technique.")
        
        # Compression features
        elif 'compression_operation' in feat_name and feat_value > 0:
            explanations.append(f"**Compression/archiving operation** (value: {value_str}, SHAP: {contribution_str}): File compression detected - may indicate data staging or exfiltration preparation.")
        
        # Discovery features
        elif 'process_discovery' in feat_name and feat_value > 0:
            explanations.append(f"**Process discovery command** (value: {value_str}, SHAP: {contribution_str}): Command lists running processes - common in reconnaissance phase.")
        
        elif 'network_discovery' in feat_name and feat_value > 0:
            explanations.append(f"**Network discovery command** (value: {value_str}, SHAP: {contribution_str}): Command enumerates network resources - reconnaissance activity.")
        
        # APT features
        elif feat_name.startswith('apt_') and feat_value > 0:
            apt_type = feat_name.replace('apt_', '').replace('_', ' ')
            explanations.append(f"**APT {apt_type} indicator** (value: {value_str}, SHAP: {contribution_str}): Detected pattern associated with {apt_type} techniques.")
        
        # Native binary features
        elif 'native_binary_abuse_score' in feat_name and feat_value > 0:
            explanations.append(f"**Native binary abuse score** (value: {value_str}, SHAP: {contribution_str}): High score indicates abuse of legitimate Windows tools for malicious purposes.")
        
        # Text embedding features
        elif feat_name.startswith('text_embedding_'):
            if 'mean' in feat_name:
                explanations.append(f"**Text embedding mean** (value: {value_str}, SHAP: {contribution_str}): Overall semantic similarity to malicious patterns in training data.")
            elif 'std' in feat_name:
                explanations.append(f"**Text embedding std** (value: {value_str}, SHAP: {contribution_str}): Variability in semantic meaning - high values indicate complex/mixed content.")
            elif 'dim_' in feat_name:
                dim_num = feat_name.split('_')[-1]
                explanations.append(f"**Text embedding dimension {dim_num}** (value: {value_str}, SHAP: {contribution_str}): Semantic feature from sentence transformer capturing command/path/context meaning.")
        
        # Generic fallback
        if not explanations:
            direction = "suggests malicious activity" if contribution > 0 and not is_benign else "indicates benign activity"
            explanations.append(f"**{feat_name.replace('_', ' ').title()}** (value: {value_str}, SHAP: {contribution_str}): This feature {direction}.")
    ...
    def load(self, directory: str):
        ...
        # Initialize SHAP explainer if available
        if SHAP_AVAILABLE and self.use_random_forest and self.random_forest.is_fitted and self.feature_names:
            try:
                self.shap_explainer = SHAPExplainer(self.random_forest, self.feature_names)
            except Exception as e:
