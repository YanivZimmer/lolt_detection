...
class LOTLEnsemble:
    ...
    def __init__(self, 
        ...
        self.random_forest = RandomForestModel(n_estimators=10,max_depth=8) if use_random_forest else None
        ...
        self.feature_extractor = ComprehensiveFeatureExtractor(use_text_embeddings=False)
    ...
    def fit(self, events: List[Dict[str, Any]], labels: List[str]):
        ...
        if self.use_random_forest:
            ...
            importances = self.random_forest.get_feature_importances().values()
            feature_imp_df = pd.DataFrame({'Feature': self.feature_names, 'Gini Importance': importances}).sort_values(
    ...
    def predict(self, events: List[Dict[str, Any]]) -> List[str]:
        ...
        # Note: LLM reasoning is currently enables (use_llm_reasoning=True)            
        ...
        if self.ensemble_method == 'weighted_vote':
            ...
            #self._generate_explanation(events[0], final_predictions[0], X[0], 0.5)
    ...
    def _generate_explanation(self, event: Dict[str, Any], prediction: str, 
        ...
        # node_indicator = self.random_forest.model.decision_path(features.reshape(1, -1))
        # used_features = np.unique(self.random_forest.get_feature_importances()[node_indicator.indices])
        # feature_names = list(features.keys())
        # importances = self.random_forest.model.feature_importances_

        # # Sort by importance (descending)
        # sorted_idx = np.argsort(importances)[::-1]

        # # Build a list of (feature_name, importance, value)
        # top_features = [
        #     {
        ...
        if self.use_random_forest and self.random_forest.is_fitted:
            ...
            if rf_explanations:
                ...
                top_contributing_features = [
                    ...
                    if abs(feat['contribution']) > 0.001
                ][:5]
        ...
        if prediction == 'malicious':
            ...
            for feat in top_features:
                feature_explanations.append(f"The {feat['name']} feature is {feat['value']:.2f} and has an importance of {feat['importance']:.2f}.")
            ...
            benign_indicators = []
            for feat in top_features:
                feature_explanations.append(f"The {feat['feature']} feature is {feat['value']:.2f} and has an importance of {feat['importance']:.2f}.")
        ...
        print(f"use_llm_reasoning: {self.use_llm_reasoning}")
        if self.use_llm_reasoning:
            llm_explanations = self.llm_distiller.generate_explanation(event, prediction)
            if llm_explanations:
                print(f"llm_explanations: {llm_explanations}")
                return llm_explanations[0]
                #explanation_parts.extend(llm_explanations)
