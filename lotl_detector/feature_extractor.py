...
class ComprehensiveFeatureExtractor:
    ...
    def _extract_sysmon_features(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        integrity_map = {"low": 1, "medium": 2, "high": 3, "system": 4}
        ...
        features["is_system_user"] = (
            1 if "SYSTEM" in user.upper() or "NT AUTHORITY" in user.upper() else 0
        )
        ...
        if image:
            ...
            features["system_account_admin_tool"] = (
                1
                if (
                    features["is_network_service"] == 1
                    and any(tool in image_lower for tool in admin_tools)
                )
                else 0
            )
        else:
        ...
        # Network features
        features["has_network_activity"] = (
            1
            if (
                event.get("SourcePort")
                or event.get("DestinationIp")
                or event.get("DestinationPort")
    ...
    def _extract_cmdline_features(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if image:
            ...
            features["is_temp_binary"] = (
                1 if "temp" in image_lower or "appdata" in image_lower else 0
            )
        ...
        features["has_encoded_content"] = (
            1 if any(pattern in cmdline_lower for pattern in ["-enc", "encoded", "%", "\\x"]) else 0
        )
        ...
        # PowerShell specific
        features["is_powershell"] = (
            1 if "powershell" in cmdline_lower or "pwsh" in cmdline_lower else 0
        )
        features["has_powershell_bypass"] = (
            1 if "bypass" in cmdline_lower and "executionpolicy" in cmdline_lower else 0
        )

        # Network-related commands
        features["has_network_cmd"] = (
            1
            if any(
                cmd in cmdline_lower
                for cmd in [
                    "curl",
                    "wget",
                    "invoke-webrequest",
        ...
        # Registry operations
        features["has_registry_ops"] = (
            1
            if any(
                cmd in cmdline_lower
                for cmd in ["reg add", "reg delete", "reg query", "reg save", "reg export"]
            )
            else 0
        )

        # Service operations
        features["has_service_ops"] = (
            1
            if any(
                cmd in cmdline_lower
                for cmd in [
        ...
        features["compression_operation"] = (
            1
            if any(
                cmd in cmdline_lower
                for cmd in ["compress-archive", "zip", "tar", "7z", "rar", "archive"]
            )
            else 0
        )
        ...
        features["process_discovery"] = (
            ...
            if any(
                cmd in cmdline_lower
                for cmd in ["tasklist", "get-process", "ps ", "wmic process", "netstat"]
            )
            else 0
        )

        # Network discovery commands
        features["network_discovery"] = (
            1
            if any(
                cmd in cmdline_lower
                for cmd in ["net view", "net share", "nslookup", "ping", "tracert"]
            )
            else 0
        )
    ...
    def _extract_process_features(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if image and parent_image:
            ...
            features["explorer_from_userinit"] = (
                1 if ("userinit.exe" in parent_exe and "explorer.exe" in image_exe) else 0
            )

            # Explorer launched by non-standard parent (should be winlogon or userinit normally)
            features["explorer_unusual_parent"] = (
                1
                if (
                    "explorer.exe" in image_exe
                    and parent_exe not in ["winlogon.exe", "userinit.exe", "explorer.exe"]
                )
                else 0
            )
            ...
            features["system_binary_from_explorer"] = (
                1
                if (
                    "explorer.exe" in parent_exe
                    and any(binary in image_exe for binary in system_binaries)
                )
                else 0
            )

            # WMI Provider launched by svchost (normal) vs other parents (suspicious)
            features["wmiprvse_unusual_parent"] = (
                1 if ("wmiprvse.exe" in image_exe and "svchost.exe" not in parent_exe) else 0
            )
        ...
        if cmdline and parent_cmdline:
            ...
            features["parent_cmdline_similarity"] = (
                1 if parent_cmdline.lower() in cmdline.lower() else 0
            )
        else:
