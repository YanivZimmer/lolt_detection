...
def compare_predictions(events: List[Dict[str, Any]], ensemble: LOTLEnsemble):
    ...
    claude_metrics = {
        ...
        "precision": precision_score(
            ground_truth_labels, claude_labels, pos_label="malicious", zero_division=0
        ),
        "recall": recall_score(
            ground_truth_labels, claude_labels, pos_label="malicious", zero_division=0
        ),
    ...
    model_metrics = {
        ...
        "precision": precision_score(
            ground_truth_labels, model_predictions, pos_label="malicious", zero_division=0
        ),
        "recall": recall_score(
            ground_truth_labels, model_predictions, pos_label="malicious", zero_division=0
        ),
...
def main():
    ...
    with open(md_path, "w") as f:
        ...
        for metric in ["accuracy", "precision", "recall", "f1"]:
            f.write(
                f"| {metric} | {comparison['claude_metrics'][metric]:.4f} | {comparison['model_metrics'][metric]:.4f} |\n"
            )
