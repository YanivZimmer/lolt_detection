...
def explain_top_features(model_path: str, data_path: str, top_k: int = 10):
    ...
    if data_path_full.exists():
        ...
        X = np.array(
            [[features.get(name, 0) for name in feature_names] for features in feature_list]
        )
    else:
...
def main():
    ...
    parser.add_argument("--data", type=str, default="data.jsonl", help="Path to dataset")
    parser.add_argument("--top-k", type=int, default=10, help="Number of top features to explain")
    parser.add_argument(
        "--output", type=str, default="feature_explanations.md", help="Output file for explanations"
    )
    ...
    try:
        top_features_explanation = explain_top_features(args.model_dir, args.data, args.top_k)
